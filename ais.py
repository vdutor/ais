...
class Model(object):
    def __init__(self, generator, prior, kernel, sigma, num_samples,
                 stepsize=0.01, n_steps=10, target_acceptance_rate=.65, avg_acceptance_slowness=0.9,
                 stepsize_min=0.0001, stepsize_max=0.5, stepsize_dec=0.98, stepsize_inc=1.02, session=None):
        """
        ...
        self.sess.run([self.stepsize.initializer, self.avg_acceptance_rate.initializer])
    ...
    def ais(self, x, schedule):
        ...
        for (t0, t1) in zip(schedule[:-1], schedule[1:]):
            ...
            w += new_u - prev_u
            # x_hat = tf.reshape(self.generator(self.z), [self.num_samples, self.batch_size, self.generator.output_dim])
            # print(self.sess.run(self.kernel.logpdf(self.x, x_hat, self.sigma), feed_dict={self.x: x, self.z: self.zv}))
...
class CModel(object):
    def __init__(self, generator, prior, kernel, sigma, num_samples,
        ...
        """
        The model performs AIS operations
        :param generator: The generator supports __call__(z, y), input_dim, output_dim
             - generator.output_dim: for mnist this = 28 * 28
             - generator.latent_dim, typically 2 or 3
             - generator.label_dim, for mnist one-hot 10
             - generator.input_dim = latent_dim + label_dim
        :param prior: The prior is the starting distribution of p(z), either p(z) or q(z|x).
        :param kernel: The kernel defines the "likelihood" of p(x|z), under the parzen approximation.
        :param sigma: Variance parameter for the parzen window
        :param num_samples: Number of samples to sample from in order to estimate the likelihood.
        ...
        self.kernel = kernel
        self.x = tf.placeholder(tf.float32, [None, self.generator.output_dim], name='x')
        self.z = tf.placeholder(tf.float32, [None, self.generator.latent_dim], name='z')
        self.y = tf.placeholder(tf.float32, [None, self.generator.label_dim], name='y')
        self.zv = None
        self.batch_size = tf.shape(self.x)[0]
        self.num_samples = num_samples
        self.sigma = sigma
        self.t = tf.placeholder(tf.float32, [], name='t')
        self.lld = tf.reshape(-self.energy_fn(self.z, self.y), [num_samples, self.batch_size])
    ...
    def step(self, x, t):
        new_z, accept, vel, _ = self.sess.run([self.new_z, self.accept, self.final_vel, self.updates],
                                              feed_dict={self.t: t, self.x: x, self.z: self.zv, self.y: self.yv})
        self.zv = new_z
        return accept

    def log_likelihood(self, x, t):
        return self.sess.run(self.lld, feed_dict={self.t: t, self.x: x, self.z: self.zv, self.y: self.yv})

    def energy_fn(self, z, y):
        mu = self.generator(z, y)
        mu = tf.reshape(mu, [self.num_samples, self.batch_size, self.generator.output_dim])
        e = self.prior.logpdf(z) + self.t * tf.reshape(self.kernel.logpdf(self.x, mu, self.sigma), [self.num_samples * self.batch_size])
        return -e

    def ais(self, x, y, schedule):
        """
        :param x: Nt x output_dim
        :param y: Nt x label_dim
        """
        w = 0.0
        self.zv = np.random.normal(0.0, 1.0, [x.shape[0] * self.num_samples, self.generator.latent_dim])
        self.yv = np.reshape(np.tile(y[:, None, :], [1, self.num_samples, 1]),
                             [x.shape[0] * self.num_samples, self.generator.label_dim])
